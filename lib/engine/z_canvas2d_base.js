...
class canvas2d_base {
    ...
    draw() {
        ...
        for (let i = 0; i < this.StringList.length; i++) {
            ...
            this.ctx.fillText(
                this.StringList[i].str,
                this.StringList[i].basePos[0],
                this.StringList[i].basePos[1]
            );
            this.ctx.strokeText(
                this.StringList[i].str,
                this.StringList[i].basePos[0],
            ...
            switch (this.pathObjList[i].type) {
                case ctxPathType.rectangle:
                    if (this.pathObjList[i].fillFlg) {
                        this.ctx.fillRect(
                            this.pathObjList[i].positionData[0],
                            this.pathObjList[i].positionData[1],
                            this.pathObjList[i].positionData[2],
                            this.pathObjList[i].positionData[3]
                        );
                    }
                ...
                case ctxPathType.circle:
                    ...
                    this.ctx.arc(
                        ...
                        this.pathObjList[i].positionData[2],
                        this.pathObjList[i].positionData[3],
                        this.pathObjList[i].positionData[4],
                        false
                    );
    ...
    _update(_delta) {
        ...
        for (let i = this.effectStringList.length - 1; i >= 0; i--) {
            ...
            if (
                this.effectStringList[i].nowTime >
                this.effectStringList[i].lifeTime
            ) {
                this.effectStringList.pop();
            } else {
                this.efTotalHeight[
                    this.effectStringList[i].positionType
                ] += this.effectStringList[i].lineHeight;
            }
        }
    }
